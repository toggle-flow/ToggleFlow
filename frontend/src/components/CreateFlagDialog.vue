        ...
        <div class="space-y-2">
          ...
          <div class="space-y-2">
            ...
            >
              <Input
                v-model="v.name"
                :placeholder="$t('flags.variationName')"
                class="flex-1"
              />
              ...
              <template v-if="flagType === 'boolean'">
                <span
                  ...
                  :class="i === 0 ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'border-rose-500/40 bg-rose-500/10 text-rose-600 dark:text-rose-400'"
                >
                  {{ i === 0 ? 'true' : 'false' }}
                </span>
              </template>
...
interface VariationForm { name: string; value: string }
const variations = ref<VariationForm[]>(defaultVariationsFor('boolean'))

function defaultVariationsFor(t: FlagType): VariationForm[] {
  if (t === 'boolean') return [{ name: 'Enabled', value: 'true' }, { name: 'Disabled', value: 'false' }]
  if (t === 'number') return [{ name: 'Variation A', value: '0' }, { name: 'Variation B', value: '1' }]
  return [{ name: 'Variation A', value: '' }, { name: 'Variation B', value: '' }]
}
...
function addVariation() {
  variations.value.push({ name: `Variation ${String.fromCharCode(65 + variations.value.length)}`, value: '' })
}
