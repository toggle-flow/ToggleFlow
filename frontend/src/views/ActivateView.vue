<template>
  <div class="min-h-screen flex flex-col items-center justify-center bg-background px-8 py-10">
    <div class="w-full max-w-sm space-y-5">
      ...
      <div class="space-y-1">
        <h2 class="text-xl font-semibold tracking-tight">{{ $t('activate.title') }}</h2>
...
onMounted(async () => {
  ...
  if (!id) return
  try {
    const info = await authApi.getInvite(id)
    inviteName.value = info.name
    inviteEmail.value = info.email
  } catch {
    // link may be invalid or already used — the form will surface the error on submit
  }
})
...
async function submit() {
  ...
  loading.value = true
  try {
    const { token: jwt, user } = await authApi.activate(token.value, password.value)
